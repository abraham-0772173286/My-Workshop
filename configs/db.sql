...
CREATE TABLE `repair_jobs` (
    ...
    `parts_cost`  DECIMAL(12,2) NOT NULL DEFAULT 0.00  COMMENT 'Cost of spare parts (KES)',
    `labour_cost` DECIMAL(12,2) NOT NULL DEFAULT 0.00  COMMENT 'Labour / service charge (KES)',
    `status`      ENUM(
                    'REPAIR PENDING',
                    'REPAIR DONE'
...
CREATE TABLE `payments` (
    ...
    `amount_paid`    DECIMAL(12,2) NOT NULL DEFAULT 0.00  COMMENT 'Amount received (KES)',
    `payment_method` ENUM(
                       'CASH',
                       'MPESA',
                       'BANK TRANSFER',
...
SELECT
    ...
    rj.parts_cost                                 AS `Parts Cost (KES)`,
    rj.labour_cost                                AS `Labour Cost (KES)`,
    (rj.parts_cost + rj.labour_cost)              AS `Total Cost (KES)`,
    rj.status                                     AS `Status`,
...
SELECT
    ...
    (rj.parts_cost + rj.labour_cost)     AS `Job Total (KES)`,
    p.amount_paid                        AS `Paid (KES)`,
...
SELECT
    ...
    SUM(rj.parts_cost + rj.labour_cost)        AS `Lifetime Value (KES)`,
...
SELECT
    ...
    (rj.parts_cost + rj.labour_cost)              AS `Amount (KES)`,
