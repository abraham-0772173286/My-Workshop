...
try {
    ...
    $status = input('status');
    $dateReceivedRaw = input('date_received');
    // Validate date; fall back to today if missing or malformed
    $dateReceived = ($dateReceivedRaw !== '' && strtotime($dateReceivedRaw) !== false)
        ? date('Y-m-d', strtotime($dateReceivedRaw))
        : date('Y-m-d');
