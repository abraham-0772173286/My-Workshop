...
try {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // viewall  – list every invoice with customer name, vehicle plate, totals
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'viewall') {
        $rows = $db->query(
            "SELECT
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // get  – single invoice with its line items (for view/print modal)
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'get') {
        $id = (int)($_GET['id'] ?? 0);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // customers_list  – lightweight list for the create-invoice dropdown
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'customers_list') {
        $rows = $db->query(
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // add  – create a new invoice + its line items
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'add') {
        ...
        // Update invoice_no to the formatted value
        ...
        foreach ($items as $item) {
            ...
            $type     = in_array($item['item_type'] ?? '', ['Labour','Part','Other']) ? $item['item_type'] : 'Labour';
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // update_status  – change paid/unpaid/cancelled
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'update_status') {
        $id     = (int) inp('id');
        $status = inp('status');
        ...
        $stmt->execute();
        reply(['status' => 'ok', 'msg' => 'Status updated.']);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // delete  – remove invoice (items cascade)
    // ─────────────────────────────────────────────────────────────────────────
    if ($f === 'delete') {
        $id = (int) inp('id');
        if ($id < 1) reply(['status' => 'error', 'msg' => 'Invalid ID.'], 422);
